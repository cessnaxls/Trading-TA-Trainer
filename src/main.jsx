...
function Chart({data,end=110,mode=0}){
 ...
 const start=Math.max(0,end-59),v=data.slice(start,end+1),n=v.length,p=14,right=58,pw=w-p-right;
 const showLine=mode===2,showEMA=[3,6,12,13,15].includes(mode),showRange=[4,5,10,13,15].includes(mode),showRSI=[7,12,13,15].includes(mode),showATR=[8,11,14,15].includes(mode),showVolume=[9,12,15].includes(mode);
 const panels=(showRSI?1:0)+(showATR?1:0)+(showVolume?1:0),mainH=230,panelH=78,gap=8,H=mainH+panels*(panelH+gap),mainTop=10,mainBottom=mainTop+mainH;
 const lo=Math.min(...v.map(x=>x.low))*.997,hi=Math.max(...v.map(x=>x.high))*1.003,X=i=>p+(i+.5)*pw/n,Y=q=>mainTop+(hi-q)/(hi-lo)*mainH,step=pw/n;
 const pts=(key,yfn=Y)=>v.map((d,i)=>`${X(i)},${yfn(d[key])}`).join(' ');
 const recent=data.slice(Math.max(0,end-34),end+1),support=Math.min(...recent.map(x=>x.low)),resistance=Math.max(...recent.map(x=>x.high));
 let panelY=mainBottom+gap;const panelDefs=[];
 if(showRSI){panelDefs.push({kind:'rsi',y:panelY});panelY+=panelH+gap}if(showATR){panelDefs.push({kind:'atr',y:panelY});panelY+=panelH+gap}if(showVolume){panelDefs.push({kind:'vol',y:panelY});panelY+=panelH+gap}
 const lineColor='#dbe7f6',ema9='#63b3ff',ema20='#ffd166',ema50='#c084fc',rangeColor='#7f93ad';
 return <div ref={ref} className="chart"><svg width={w} height={H}><rect width={w} height={H} rx="12" className="bg"/>
 {[0,1,2,3,4].map(k=><line key={'g'+k} x1={p} x2={p+pw} y1={mainTop+k*mainH/4} y2={mainTop+k*mainH/4} className="grid"/>)}
 {showLine?<polyline points={pts('close')} fill="none" stroke={lineColor} strokeWidth="2.2"/>:v.map((d,i)=>{let up=d.close>=d.open,bw=Math.max(2,step*.55);return <g key={i}><line x1={X(i)} x2={X(i)} y1={Y(d.high)} y2={Y(d.low)} className={up?"wick up":"wick down"}/><rect x={X(i)-bw/2} y={Math.min(Y(d.open),Y(d.close))} width={bw} height={Math.max(1,Math.abs(Y(d.open)-Y(d.close)))} className={up?"candle up":"candle down"}/></g>})}
 {showEMA&&<><polyline points={pts('ema9')} fill="none" stroke={ema9} strokeWidth="1.8"/><polyline points={pts('ema20')} fill="none" stroke={ema20} strokeWidth="1.8"/><polyline points={pts('ema50')} fill="none" stroke={ema50} strokeWidth="1.8"/><text x={p+6} y={mainTop+15} fill={ema9} fontSize="10">EMA9</text><text x={p+49} y={mainTop+15} fill={ema20} fontSize="10">EMA20</text><text x={p+99} y={mainTop+15} fill={ema50} fontSize="10">EMA50</text></>}
 {showRange&&<><line x1={p} x2={p+pw} y1={Y(resistance)} y2={Y(resistance)} stroke={rangeColor} strokeDasharray="5 4"/><line x1={p} x2={p+pw} y1={Y(support)} y2={Y(support)} stroke={rangeColor} strokeDasharray="5 4"/><text x={p+pw+5} y={Y(resistance)+3} fill={rangeColor} fontSize="9">R</text><text x={p+pw+5} y={Y(support)+3} fill={rangeColor} fontSize="9">S</text></>}
 <text x={p+pw+5} y={Y(v[n-1].close)+3} fill="#dbe7f6" fontSize="9">{fmt(v[n-1].close,1)}</text>
 {panelDefs.map(pd=>{if(pd.kind==='rsi'){const py=pd.y,yy=q=>py+(100-q)/100*panelH;return <g key="rsi"><rect x={p} y={py} width={pw} height={panelH} fill="#081522" stroke="#1b2b40"/><line x1={p} x2={p+pw} y1={yy(70)} y2={yy(70)} stroke="#455a73" strokeDasharray="4 4"/><line x1={p} x2={p+pw} y1={yy(30)} y2={yy(30)} stroke="#455a73" strokeDasharray="4 4"/><polyline points={pts('rsi',yy)} fill="none" stroke="#65d1ff" strokeWidth="1.8"/><text x={p+5} y={py+13} fill="#9bb0c9" fontSize="9">RSI 14</text><text x={p+pw+5} y={yy(v[n-1].rsi)+3} fill="#65d1ff" fontSize="9">{fmt(v[n-1].rsi,0)}</text></g>}
 if(pd.kind==='atr'){const py=pd.y,vals=v.map(d=>d.atr/d.close*100),mx=Math.max(2.4,...vals)*1.08,yy=q=>py+(mx-q)/mx*panelH;return <g key="atr"><rect x={p} y={py} width={pw} height={panelH} fill="#081522" stroke="#1b2b40"/><line x1={p} x2={p+pw} y1={yy(.8)} y2={yy(.8)} stroke="#455a73" strokeDasharray="4 4"/><line x1={p} x2={p+pw} y1={yy(1.8)} y2={yy(1.8)} stroke="#455a73" strokeDasharray="4 4"/><polyline points={v.map((d,i)=>`${X(i)},${yy(vals[i])}`).join(' ')} fill="none" stroke="#f4a261" strokeWidth="1.8"/><text x={p+5} y={py+13} fill="#9bb0c9" fontSize="9">ATR % of price • 0.8 compressed / 1.8 high</text><text x={p+pw+5} y={yy(vals[n-1])+3} fill="#f4a261" fontSize="9">{fmt(vals[n-1],2)}%</text></g>}
 const py=pd.y,maxV=Math.max(...v.map(d=>d.volume))*1.12,yy=q=>py+(maxV-q)/maxV*panelH,avg=v.slice(Math.max(0,n-20),n-1).reduce((a,d)=>a+d.volume,0)/Math.max(1,Math.min(20,n-1));return <g key="vol"><rect x={p} y={py} width={pw} height={panelH} fill="#081522" stroke="#1b2b40"/>{v.map((d,i)=><rect key={i} x={X(i)-Math.max(1,step*.25)} y={yy(d.volume)} width={Math.max(2,step*.5)} height={py+panelH-yy(d.volume)} fill={d.close>=d.open?"#2d8b68":"#9a4655"}/>) }<line x1={p} x2={p+pw} y1={yy(avg)} y2={yy(avg)} stroke="#9bb0c9" strokeDasharray="4 4"/><text x={p+5} y={py+13} fill="#9bb0c9" fontSize="9">Volume • dashed = recent average</text></g>})}
 </svg></div>
}
...
function qFor(li,idx,seed){
 ...
 const choice=(prompt,choices,answer,explain)=>({type:"choice",prompt,choices,answer,explain,data,end,mode:li});
 const probChoice=(prompt,target,explain)=>{
   ...
   return {type:"choice",prompt,choices:bands,answer:ans,explain:data?`${explain} Target probability: ${target}%, which falls in ${ans}.`:explain,data,end,mode:li};
 };
 switch(li){
  case 0:{
  ...
  case 2:{let a0=data[end-40].close,a1=data[end-20].close,a2=x.close,up=a2>a1&&a1>a0,down=a2<a1&&a1<a0,a=up?"Bullish":down?"Bearish":"Mixed/ranging";return choice("Classify the dominant closing-price structure.",["Bullish","Bearish","Mixed/ranging"],a,`The visible closing-price line progresses ${up?"higher":down?"lower":"without a consistent directional sequence"}.`);}
...
function Question({q,i,value,onChange,disabled,gradeResult}){
 return <div className={"question "+(gradeResult===true?"correct":gradeResult===false?"wrong":"")}><div className="qhead"><b>Question {i+1}{Number.isInteger(q.topic)?` • ${lessons[q.topic][0]}`:""}</b>{gradeResult!=null&&<span>{gradeResult?"✓ Correct":"✕ Incorrect"}</span>}</div><Chart data={q.data} end={q.end} mode={q.mode}/><p>{q.prompt}</p>
...
function App(){
 ...
 <section className="panel"><div className="sectionTitle"><div><span>RANDOMIZED WORKED EXAMPLES</span><h2>Chart examples</h2></div><button onClick={()=>setSeed(Date.now()%1e9)}>Randomize</button></div>{examples.map((q,i)=><div className="example" key={i}><h3>Example {i+1}</h3><Chart data={q.data} end={q.end} mode={q.mode}/><p><b>Question:</b> {q.prompt}</p><p className="solution"><b>Solution:</b> {q.explain}</p></div>)}</section>
